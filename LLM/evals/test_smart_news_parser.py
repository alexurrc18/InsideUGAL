...
class TestSmartNewsParserStandalone:
    ...
    def test_health_check_contine_service_name(self, client):
        ...
        assert "Smart Announcement Parser" in data["service"]

    def test_extract_announcement_info_returneaza_200(self, client):
        resp = client.post("/api/v1/extract-announcement-info", json={"text": ANUNT_EXAMEN})
        assert resp.status_code == 200

    def test_extract_announcement_info_schema_completa(self, client):
        resp = client.post("/api/v1/extract-announcement-info", json={"text": ANUNT_EXAMEN})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_actiuni_extrase_nevide(self, client):
        resp = client.post("/api/v1/extract-announcement-info", json={"text": ANUNT_EXAMEN})
        assert resp.status_code == 200
        actiuni = resp.json()["actiuni_extrase"]
        assert isinstance(actiuni, list)
        assert len(actiuni) > 0
    ...
    def test_text_prea_scurt_returneaza_422(self, client):
        resp = client.post("/api/v1/extract-announcement-info", json={"text": "scurt"})
        assert resp.status_code == 422

    def test_text_prea_lung_returneaza_422(self, client):
        text_lung = "a" * 5001
        resp = client.post("/api/v1/extract-announcement-info", json={"text": text_lung})
        assert resp.status_code == 422

    def test_anunturi_diferite_produc_rezultate_diferite(self, client):
        resp1 = client.post("/api/v1/extract-announcement-info", json={"text": ANUNT_EXAMEN})
        resp2 = client.post("/api/v1/extract-announcement-info", json={"text": ANUNT_CONCURS})
