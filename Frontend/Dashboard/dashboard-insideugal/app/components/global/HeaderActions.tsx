    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-card border border-border rounded-xl shadow-lg p-4 z-50 max-h-96 overflow-y-auto">
          <h3 className="font-bold text-sm text-foreground mb-3">Notificări și Anunțuri</h3>
          {announcements.length === 0 ? (
            <p className="text-xs text-muted text-center py-4">Nu există anunțuri noi.</p>
          ) : (
            <div className="space-y-3">
              {announcements.map((ann) => (
                <div key={ann.id} className="border-b border-border/50 pb-2 last:border-0 last:pb-0">
                  ...
                  <p className="text-[11px] text-muted mt-0.5 line-clamp-2">{ann.description}</p>
                </div>
              ))}
            </div>
          )}
